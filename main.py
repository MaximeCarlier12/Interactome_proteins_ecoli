...
def quant_analysis(LFQ_bool, normalization_method):
  ...
  common_variances = get_common_variances_per_media(prot_3_rep, threshold, LFQ_bool, normalization_method) # calculation of the common variance for each media condition (list of 3 values).
  for prot in prot_3_rep: # loop for each condition used.
    ...
    qt.test_normal_equal_var(prot, threshold_med, LFQ_bool, normalization_method, common_variances) # calculate tests for each protein and add it to file table.
    qt.plot_log10_abundance(prot, threshold_med, LFQ_bool, normalization_method, common_variances) # plot log10(normalized values)
  ...
  for prot_name in PROTEINS:
    create_putative_proteins_analysis(prot_3_rep, prot_name, False, 1) # create 1 file per bait protein that contains the name of all enriched prey proteins. There is one sheet per growth condition (i.e 3 sheets).
